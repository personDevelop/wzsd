...
function useOrCancelBalance(elelment) {
    ...
    if ($(elelment).prop("checked")) {
        ...
        var PayMoney = total - useCouponValue; 
          balance = $("#canUsedBalanceId").find(".ftx-01").text() * 1;
        if (balance > PayMoney) {
...
$(function () {
    ...
    $("#order_UserBalance").bind("change", function () {
        var canUserBalance = $("#canUsedBalanceId").attr("value") * 1;  
        var total = $("#warePriceId").attr("value") * 1;
        var useCouponValue = $("#useCouponValue").attr("value") * 1;
        var PayMoney = total - useCouponValue;
        var minValue = Math.min.apply(null, [canUserBalance, PayMoney]);
        ...
        else { balance = 0; $(this).val(balance); }
        if (balance > minValue) {
            balance = minValue;
            $(this).val(balance);
        ...
        $("#useBalanceLabel").text(balance); ComputPrice();
    });
    ...
    $(".coupons").delegate("li", "click", function () {
        ...
        if (!$(this).hasClass("selected")) {
            ...
            $("#order_Coupon_0__ID").val($(this).attr("couponID"));
            $("#order_Coupon_0__UsingCount").val(1);
            value = $(this).attr("value") * 1;
           
        } else {
            $(this).removeClass("selected")
            $("#order_Coupon_0__ID").val("");
            $("#order_Coupon_0__UsingCount").val("");
            
        }
        if (value > 0) {
            $("#useCouponCount").text("1");
        } else {
            $("#useCouponCount").text("0");
        }
        $("#useCouponValue").text(value);
        $("#useCouponValue").attr("value", value);
        ComputPrice();
    });
