...
namespace AliNotify.Web
{
    ...
    public class NotifyUtil
    {
        ...
        public NotifyUtil(string orderID)
        {

            //初始化基础配置信息
            config = new PayBill().GetAliPayConfig(orderID, out Error);
        ...
        private bool GetSignVeryfy(SortedDictionary<string, string> inputPara, string sign)
        {
            ...
            if (!string.IsNullOrWhiteSpace(sign))
            {
                ...
                {
                    case "RSA":
                        isSgin = RSAFromPkcs8.verify(preSignStr, sign, config.PublicKey, config.input_charset);
                        break;
                    default:
                        break;
                }
